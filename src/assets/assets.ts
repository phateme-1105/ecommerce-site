...
export const getBestSeller= async (): Promise<Product[]> => {
return Promise.resolve(products.filter(p => p.bestseller).slice(0, 5))
}

export const getProduct = async (id: string): Promise<Product | undefined> => {
return Promise.resolve(products.find(product => product._id === id))}

export const getRelatedProducts = async (Category:string, type:string, id:string): Promise<Product[]| undefined> => {
return Promise.resolve(products.filter(p => p.category === Category && p.subCategory === type && p._id !== id).slice(0, 5))
}